...
(defn- emit-router [router-id sym union-sym]
  `(om.next/defui ~(vary-meta sym assoc :once true)
     ~'static untangled.client.core/InitialAppState
     (~'initial-state [~'clz ~'params] {:id ~router-id :current-route (untangled.client.core/get-initial-state ~union-sym {})})
     ~'static om.next/Ident
     (~'ident [~'this ~'props] [:untangled.client.routing.routers/by-id ~router-id])
     ~'static om.next/IQuery
